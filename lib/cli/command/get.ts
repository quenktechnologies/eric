...
import {
  ...
  pure,
  parallel, 
  liftP } from '@quenk/noni/lib/control/monad/future';
...
const cleanup = (path: Path) =>
    unlink(join(path, FILE_GIT))
        .chain(() => exists(join(path, FILE_ERIC_JSON)))
        .chain(yes => yes ? unlink(join(path, FILE_ERIC_JSON)) : pure(noop()))
        .map(noop);
